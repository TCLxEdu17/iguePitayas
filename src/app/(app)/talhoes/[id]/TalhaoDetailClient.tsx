...
interface Plot {
  ...
  name: string
  area: number | null
  productType: keyof typeof PRODUCT_LABELS
  status: string
  notes: string | null
...
export default function TalhaoDetailClient({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)

  const { data: plot, isPending, isError } = useQuery<Plot>({
    queryKey: ['plot', id],
    queryFn: () =>
